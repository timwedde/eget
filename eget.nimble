...
task eget, "Executes 'nimble run' with extra compiler options.":
  ...
  exec(&"nimble --mm:orc run eget {args}")

task build_macos_arm64, "Builds for macOS (arm64)":
  exec("nimble build -d:danger --opt:size --mm:orc --os:macosx --cpu:arm64 -d:strip -y")
...
task build_macos_amd64, "Builds for macOS (amd64)":
  exec("nimble build -d:danger --opt:size --mm:orc --os:macosx --cpu:amd64 -d:strip -y")
...
task build_linux_amd64, "Builds for linux (amd64)":
  exec("nimble build -d:danger --opt:size --mm:orc --os:linux --cpu:amd64 -d:strip -y")
...
task build_linux_i386, "Builds for linux (i386)":
  exec("nimble build -d:danger --opt:size --mm:orc --os:linux --cpu:i386 -d:strip -y")
...
task build_windows_amd64, "Builds for Windows (amd64)":
  exec("nimble build -d:danger --opt:size --mm:orc -d:mingw --cpu:amd64 -d:strip -y")
